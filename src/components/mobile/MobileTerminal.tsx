...
export const MobileTerminal = () => {
    ...
    const handleSubmit = () => {
        ...
        const result: Line[] = handler ? handler() : [{ type: 'error', text: `command not found: ${cmd}\nType 'help' to see available commands.` }];
    ...
    return (
        <div className={styles.terminal} onClick={() => inputRef.current?.focus()}>
            <div className={styles.header}>
                <span className={styles.headerTitle}>Terminal — zsh</span>
            </div>

            <div className={styles.body}>
                {lines.map((line, i) => (
                    <div key={i} className={styles[line.type]}>
                        {line.type === 'input' && <span className={styles.prompt}>inho@portfolio ~ % </span>}
                        <span>{line.text}</span>
                    </div>
                ))}
